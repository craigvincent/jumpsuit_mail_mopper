...
public class ReviewApp
{
    ...
    {
        AnsiConsole.Clear();
        AnsiConsole.Write(new Rule($"[bold blue]{Markup.Escape(sender.From)}[/] — {sender.Classifications.Count} emails").LeftJustified());
        ...
        if (sender.Classifications.Count > 25)
            emailTable.Caption($"[dim]Showing 25 of {sender.Classifications.Count} emails[/]");

        AnsiConsole.Write(emailTable);
        AnsiConsole.WriteLine();

        AnsiConsole.MarkupLine($"  [red]T[/]=trash  [green]K[/]=keep  [cyan]W[/]=whitelist domain  [yellow]B[/]=back");
